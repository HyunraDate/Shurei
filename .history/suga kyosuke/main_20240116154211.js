...
for (let i = 0; i < n; i++){
    let r = 0.5 //半径
    const geometry = new THREE.BoxGeometry(0.1, 0.1, 0.5) //z是轮胎宽度，let it 形成一个圈
    const mesh = new THREE.Mesh(geometry,material)
    mesh.position.x= r * Math.cos(Math.PI * 2 / n ) //PI: 180度， PI *2 ： 360 度
    mesh.position.Y= r * Math.cos(Math.PI * 2 / n )
