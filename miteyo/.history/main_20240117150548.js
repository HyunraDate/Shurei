...
//文件夹
const folder = gui.addFolder('cam position')

//These are all object
gui.add(camera.position, 'x', -5, 5, 0.01).name('cam position -x') //gui控制相机范围x,y,z; 还可以调整路径
gui.add(camera.position, 'y', -5, 5, 0.01).name('cam position -y')
gui.add(camera.position, 'z', -5, 5, 0.01).name('cam position -z')
