...
import { someFunction } from '../js/Road.js';
...
//材质球球的统一规格
const meshSphere1 = new THREE.Mesh(sphereG, material2)
meshSphere1.position.y = 2
scene.add(meshSphere1)


//Light - Lamp, Sunlight
const light = new THREE.AmbientLight(0xffffff, 0.9) //环境光 氛围光 for 整体 //0.2是强度
scene.add(light)
...
//Camera - 
let camera = new THREE.PerspectiveCamera(75, w/h, 0.1, 100) //75 is angle, size, how long u can see.
//if u wanna resize, you have to change here"const" to "let"
...
//resize
window.addEventListener('resize', ()=>{ //重新更改宽高
    w = window.innerWidth;
    h = window.innerHeight;

    //Camera
    camera.aspect = w / h;//重新设置相机size
    camera.updateProjectionMatrix(); //自动更新
    console.log("change");

    //Renderer
    renderer.setSize(w, h); //Renderer也要重新设置
})
