...
//Material
const material = new THREE.MeshStandardMaterial({color: 0xffa1c7}) 
...
dLight.castShadow = true //这个光线的阴影
dLight.shadow.mapSize.width = 2048 //分辨率越大 阴影表面more光滑
dLight.shadow.mapSize.height = 2048 //but 计算量会大d
...
//PointLight(点光源)：类似生活中的灯泡，光纤(光芒)没有固定方向，朝着四周散射。
//*注意：点光源对应的辅助对象 PointLightHelper 只有一个 菱形的光源形状，并没有 光 的发射线条。
// const pointLight = new THREE.PointLight(0xffffff)
// scene.add(pointLight)
// const pointLightHelper = new THREE.PointLightHelper(pointLight)
// pointLight.position.y = 1 //从上面打下来
// pointLight.intensity = 1
// pointLight.distance = 2
// scene.add(pointLightHelper)


//RectAreaLight only effect to MeshStandMaterial / MeshPhysicalMaterial
//effect like 从中间劈开
//* need to import {RectAreaLightHelper}
// const rectAreaLight = new THREE.RectAreaLight(0xffffff, 1, 1, 1)
// rectAreaLight.position.set(0, 1, 0)
// rectAreaLight.rotation.x = -0.5 * Math.PI //like a 打光板
// scene.add(rectAreaLight)

// const helper = new RectAreaLightHelper(rectAreaLight) //因为是直接导入import的 就不用继续搞了
// scene.add(helper) 


//HemisphereLight(半球光源)：need skyColor & backgroundColor
//相对 ambientLight，hemisphereLight 更加真实的模拟自然光源，提供 天空 和 地面 漫反射光线。
//一共接收 3 个参数：
//第 1 个参数：天空光线颜色; 第 2 个参数：地面反射光颜色；第 3 个参数：光的反射强度
//*注意：不支持阴影
