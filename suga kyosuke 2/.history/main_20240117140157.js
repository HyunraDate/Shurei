...
//CylinderGeometry 圆柱的参数 (radiusTop : Float, radiusBottom : Float, height : Float, 
...
const planeM = new THREE.MeshBasicMaterial({ color:'rgb(255,255,227)'})
const plane = new THREE.Mesh(planeG, planeM)
scene.add(plane)
plane.rotation.x = -0.5 * Math.PI //正90度会看不到

car.rotation.x = -0.5 * Math.PI
car.position.y = 0.6 //因为轮子高度是1
...
//camera.position.set(0, 0, 5) this is 默认值
...
function tick(){ 
    ...
    frontWheels.rotation.x = time // 沿着x轴自转
    backWheels.rotation.x = time 
    
    car.position.z= time % 2
